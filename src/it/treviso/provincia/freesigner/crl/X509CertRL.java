 ...
 *	Copyright (c) 2005 Francesco Cendron, Leopoldo Consiglio - Infocamere
 *
...
/*
 */
package it.treviso.provincia.freesigner.crl;
...
public class X509CertRL {
        ...
        try {
            ...
            if (crl.getVersion() >= 1) {
                ...
                try {
                    ...
                    if (extVal != null) {
                        ...
                        if (reason.endsWith("6")) { // ReasonFlags.CERTIFICATEHOLD
                            ...
                            if (date.before(revDate)) {
                                trace("Il certificato risulta sospeso alla data: "
                                        + revDate);
                                ...
                                trace("Il certificato risulta sospeso in data: "
                                        + revDate);
                    ...
                    trace(ex);
                    trace("isNotRevoked - Errore nella lettura delle estensioni di revoca -> "
                            + ex.getMessage());

                    return false;
                }
                // la versione della CRL e' la uno e quindi non si può
                // distinguere
    ...
    public boolean update(X509Certificate userCert, Date date,
        ...
        if (forceUpdate) {
            ...
            trace("Try getting CRL from filesystem ...");
            crl = loadCRL(userCert);
            if (crl == null)
                trace("CRL not available on filesystem.");
            else {
                trace("CRL loaded, nextUpdate: " + crl.getNextUpdate());
                if (crl.getNextUpdate().before(date)) {
                    trace("CRL expired, discarding it");
                    crl = null;
                }
            }
            ...
            X509Certificate crlIssuerCert = certAuths.getCACertificate(issuer);

            int verCode = check(crl, crlIssuerCert, date);
     ...
     */
    public static URL[] getCrlDistributionPointOLD(X509Certificate certificate)
            throws CertificateParsingException {
        try {
            // trova i DP (OID="2.5.29.31") nel certificato
            DERObject obj = getExtensionValue(certificate, "2.5.29.31");
    ...
    private void storeCRL(X509CRL crl) {
        try {
            ...
            curDir + System.getProperty("file.separator") + "crl"
                    + System.getProperty("file.separator")
                    + getCommonName(crl.getIssuerX500Principal().getName()) + ".crl";
            ...
            trace("CRL salvata in: " + filePath);
            
        } catch (Exception e) {
            trace("Errore nel salvataggio su file della CRL: " + e);
        }
    }
    ...
    private X509CRL loadCRL(X509Certificate userCert) {
        ...
        try {
            ...
            curDir + System.getProperty("file.separator") + "crl"
                    + System.getProperty("file.separator")
                    + getCommonName(userCert.getIssuerX500Principal().getName()) + ".crl";

            crl = parse(getBytesFromPath(filePath));
	  } catch (FileNotFoundException fnfe) {
            trace("File CRL non presente: " + fnfe.getMessage());
        } catch (IOException ioe) {
            trace("Lettura file: " + ioe.getMessage());
        } catch (GeneralSecurityException gse) {
            trace("Parsing CRL: " + gse.getMessage());
        }
    ...
    public X509CRL download(String crlDP, Principal issuer)
        ...
        if (protocol.equalsIgnoreCase("ldap")) {
            ...
            trace("isNotRevoked - protocollo di accesso alla CRL non supportato: "
                    + protocol);
            return null;
        }
    }
    ...
    private boolean initHTTPS() {
        ...
        if (1.2 <= dVersion.doubleValue()) {
            ...
            try {
                ...
                trace("Classi di JSSE SSL non trovate. Controllare il classpath: "
                        + cfe.toString());
                return false;
            } catch (Exception e) {
                trace("Errore generico nel metodo initHTTPS --> "
                        + e.toString());
                return false;
            }
        } else {
    ...
    private X509CRL parse(byte[] crlEnc) throws GeneralSecurityException {
        ...
        try {
            ...
            // crlData = new sun.misc.BASE64Decoder().decodeBuffer(new
            // String(crlEnc));
    ...
    public static X509Certificate getCertificatesFromFile(String filePath) {
        ...
        try {
            ...
            try {
                ...
                // quindi è in DER (again italian law!)
