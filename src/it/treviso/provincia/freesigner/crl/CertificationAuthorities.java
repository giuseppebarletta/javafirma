...
/**
 * CA (Certification Authorities) that issues digital certificates <BR>
 * (also called digital passports, X.509 certificates, or public-key
 * certificates).<BR>
 * This class allows you to add, delete and get information about a CA.<BR>
 * It also puts in service a CRL manager that verifies and controls certificate
 * revocation lists.<BR>
 * <BR>
 * 
...
public class CertificationAuthorities {
    ...
    public CertificationAuthorities(File caDir, boolean debug)
        ...
        if (!caDir.isDirectory()) {
            trace(caDir.getPath() + " non e' una directory");
            throw new IllegalArgumentException(caDir.getPath()
                    + " non e' una directory");
        } else if (!caDir.canRead()) {
            trace(caDir.getPath() + " non e' leggibile");
            throw new IllegalArgumentException(caDir.getPath()
                    + " non e' leggibile");
        } else {
            String nome = null;
            ...
            for (int i = 0; i < certFiles.length; i++) {
                ...
                trace("Lettura del file: " + nome);
                try {
                    addCertificateAuthority(getBytesFromPath(nome));
                } catch (GeneralSecurityException ge) {
                    trace("Certificato CA non valido: " + nome + " - "
                            + ge.getMessage());
                }
            }
        }
    ...
    public boolean verify(X509Certificate userCert, Date date)
        ...
        if (!isAccepted(userCert)) {
            setMessage("Certificato non emesso da una CA accettata");
            return false;
        }
        ...
        try {
            ...
            trace((userCert.getSubjectDN()) + " valido fino al "
                    + userCert.getNotAfter());
        } catch (CertificateExpiredException e) {
            setMessage("Certificato scaduto il " + userCert.getNotAfter());
            return false;
        } catch (CertificateNotYetValidException e) {
            setMessage("Certificato valido dal " + userCert.getNotBefore());
            return false;
        ...
        try {
            ...
            trace("Verifica validita' con il certificato di CA OK");
            return true;
        } catch (GeneralSecurityException gse) {
            trace(gse);
            setMessage("Verifica di firma del certificato: "
    ...
    /**
     * Set proxy connection parameters to download CRL Imposta i parametri di
     * connessione con il proxy verso Internet per lo scarico delle CRL
     * 
     * @param proxy
    ...
    public boolean setUseproxy(boolean proxy, String user, String password,
        ...
        return crls.setUseproxy(proxy, user, password, proxyHost, proxyPort);
    }
    ...
    public void save() throws Exception {
        try {
            ...
            while (it.hasNext()) {
                ...
                // System.out.println("Adding: " + cert.getIssuerDN());
